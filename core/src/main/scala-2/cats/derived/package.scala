...
object auto {
  ...
  object empty {
    implicit def kittensMkEmpty[A](implicit
        @nowarn("cat=unused") refute: Refute[Empty[A]],
        ev: Lazy[MkEmpty[A]]
    ): Empty[A] = ev.value
  }
  ...
  object emptyK {
    implicit def kittensMkEmptyK[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[EmptyK[F]],
        F: MkEmptyK[F]
    ): EmptyK[F] = F
  }
  ...
  object eq {
    implicit def kittensMkEq[A](implicit
        @nowarn("cat=unused") refute: Refute[Eq[A]],
        ev: Lazy[MkEq[A]]
    ): Eq[A] = ev.value
  }
  ...
  object partialOrder {
    implicit def kittensMkPartialOrder[A](implicit
        @nowarn("cat=unused") refute: Refute[PartialOrder[A]],
  ...
  object order {
    implicit def kittensMkOrder[A](implicit
        @nowarn("cat=unused") refute: Refute[Order[A]],
        ev: Lazy[MkOrder[A]]
    ): Order[A] = ev.value
  }
  ...
  object hash {
    implicit def kittensMkHash[A](implicit
        @nowarn("cat=unused") refute: Refute[Hash[A]],
        ev: Lazy[MkHash[A]]
    ): Hash[A] = ev.value
  }
  ...
  object invariant {
    implicit def kittensMkInvariant[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Invariant[F]],
        F: MkInvariant[F]
    ): Invariant[F] = F
  }
  ...
  object contravariant {
    implicit def kittensMkContravariant[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Contravariant[F]],
        F: MkContravariant[F]
    ): Contravariant[F] = F
  }
  ...
  object functor {
    implicit def kittensMkFunctor[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Functor[F]],
        F: MkFunctor[F]
    ): Functor[F] = F
  }
  ...
  object apply {
    implicit def kittensMkApply[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Apply[F]],
        F: MkApply[F]
    ): Apply[F] = F
  }
  ...
  object applicative {
    implicit def kittensMkApplicative[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Applicative[F]],
        F: MkApplicative[F]
    ): Applicative[F] = F
  }
  ...
  object show {
    implicit def kittensMkShow[A](implicit
        @nowarn("cat=unused") refute: Refute[Show[A]],
        ev: Lazy[MkShow[A]]
    ): Show[A] = ev.value
  }
  ...
  object semigroup {
    implicit def kittensMkSemigroup[A](implicit
        @nowarn("cat=unused") refute: Refute[Semigroup[A]],
  ...
  object commutativeSemigroup {
    implicit def kittensMkCommutativeSemigroup[A](implicit
        @nowarn("cat=unused") refute: Refute[CommutativeSemigroup[A]],
  ...
  object monoid {
    implicit def kittensMkMonoid[A](implicit
        @nowarn("cat=unused") refute: Refute[Monoid[A]],
        ev: Lazy[MkMonoid[A]]
    ): Monoid[A] = ev.value
  }
  ...
  object commutativeMonoid {
    implicit def kittensMkCommutativeMonoid[A](implicit
        @nowarn("cat=unused") refute: Refute[CommutativeMonoid[A]],
  ...
  object semigroupK {
    implicit def kittensMkSemigroupK[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[SemigroupK[F]],
        F: MkSemigroupK[F]
    ): SemigroupK[F] = F
  }
  ...
  object monoidK {
    implicit def kittensMkMonoidK[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[MonoidK[F]],
        F: MkMonoidK[F]
    ): MonoidK[F] = F
  }
  ...
  object foldable {
    implicit def kittensMkFoldable[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Foldable[F]],
        F: MkFoldable[F]
    ): Foldable[F] = F
  }
  ...
  object reducible {
    implicit def kittensMkReducible[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Reducible[F]],
        F: MkReducible[F]
    ): Reducible[F] = F
  }
  ...
  object traverse {
    implicit def kittensMkTraverse[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Traverse[F]],
        F: MkTraverse[F]
    ): Traverse[F] = F
  }
  ...
  object nonEmptyTraverse {
    implicit def kittensMkNonEmptyTraverse[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[NonEmptyTraverse[F]],
  ...
  object pure {
    implicit def kittensMkPure[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[Pure[F]],
        F: MkPure[F]
    ): Pure[F] = F
  }
  ...
  object consK {
    implicit def kittensMkConsK[F[_]](implicit
        @nowarn("cat=unused") refute: Refute[ConsK[F]],
        F: MkConsK[F, F]
